...
class MPVPlayer(
    ...
    init {
        ...
        val internalMpvDir = File(context.filesDir, "mpv")
        if (!internalMpvDir.exists()) {
            internalMpvDir.mkdirs()
        }
        ...
        // internal scripts
        arrayOf("skip-intro.lua").forEach { fileName ->
            val file = File(internalMpvDir, fileName)
            val tmp = context.assets.open(fileName, AssetManager.ACCESS_STREAMING)
            if (!file.exists() || file.length() != tmp.available().toLong()) {
                tmp.copyTo(FileOutputStream(file, false))
            }
        }
        ...
        MPVLib.setOptionString("scripts", File(internalMpvDir.path, "skip-intro.lua").path)
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            MPVLib.setOptionString("override-display-fps", "$refreshRate")
            MPVLib.setOptionString("vo-null-fps", "$refreshRate")
        }
        ...
        arrayOf(
            ...
            Property("core-idle", MPVLib.MPV_FORMAT_FLAG),
    ...
    fun skipForward() {
        setPlayerStateAndNotifyIfChanged(playbackState = Player.STATE_BUFFERING)
        MPVLib.command(arrayOf("script-binding", "skip-key"))
    }
    ...
    override fun prepare() {
        internalMediaItems?.firstOrNull { it.localConfiguration?.uri != null }?.let { mediaItem ->
            ...
            setPlayerStateAndNotifyIfChanged(playbackState = Player.STATE_BUFFERING)
            MPVLib.command(arrayOf("loadfile", "${mediaItem.localConfiguration?.uri}"))
            MPVLib.setPropertyBoolean("pause", true)
            listeners.sendEvent(Player.EVENT_TIMELINE_CHANGED) { listener ->
    ...
    /**
     * Stops playback without resetting the player. Use [.pause] rather than this method if
     ...
     *
     * @see com.google.android.exoplayer2.Player.Listener.onTrackSelectionParametersChanged}
     */
     ...
     * @see com.google.android.exoplayer2.Player.Listener.onSeekBackIncrementChanged
     */
     ...
     * @see com.google.android.exoplayer2.Player.Listener.onSeekForwardIncrementChanged
     */
    ...
    /**
     * Returns the maximum position for which [.seekToPrevious] seeks to the previous window,
     * in milliseconds.
     *
     * @return The maximum seek to previous position, in milliseconds.
     * @see com.google.android.exoplayer2.Player.Listener.onMaxSeekToPreviousPositionChanged
     */
    ...
    companion object {
        ...
        /**
         * Merges multiple [MediaSource]'s into a single [MediaItem] when required.
         */
        fun parseMediaSource(mediaSource: MediaSource) : MediaItem {
            return if (mediaSource !is MergingMediaSource) {
                mediaSource.mediaItem
            } else {
                val subtitles = mutableListOf<MediaItem.SubtitleConfiguration>()
                mediaSource.getPrivateProperty<MergingMediaSource, Array<*>>("mediaSources")?.forEach { source ->
                    if (source is SingleSampleMediaSource) {
                        source.mediaItem.localConfiguration?.subtitleConfigurations?.forEach { subtitle ->
                            source.getPrivateProperty<SingleSampleMediaSource, Format>("format")?.id?.let { id ->
                                subtitles.add(subtitle.buildUpon().setLabel(id).build())
                            }
                        }
                    }
                }
                mediaSource.mediaItem.buildUpon().setSubtitleConfigurations(subtitles).build()
            }
        }

        @Suppress("UNCHECKED_CAST")
        private inline fun <reified T : Any, R> T.getPrivateProperty(name: String): R? = try {
