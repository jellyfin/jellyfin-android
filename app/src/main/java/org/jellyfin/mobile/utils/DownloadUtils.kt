...
class DownloadUtils(val context: Context, private val filename: String, private val downloadURL: String) : KoinComponent {
    ...
    private suspend fun downloadThumbnail() {
        ...
        val bitmap: Bitmap? = imageLoader.execute(imageRequest).drawable?.toBitmap()
    ...
    private suspend fun createDownloadNotification() {
        ...
        notificationBuilder = NotificationCompat.Builder(context, Constants.DOWNLOAD_NOTIFICATION_CHANNEL_ID).apply {
            ...
            setOngoing(true)
            setContentText("${context.getString(R.string.downloading)} ${jellyfinMediaSource?.name}")
